...
class TaskOrchestrator(
    ...
    fun cancelCurrentTask() {
        ...
        val cancelledSession = taskSessionStore.snapshot()
        
        if (::agentService.isInitialized) {
            agentService.cancel()
        }
        
        // Force immediate release of the lock to unblock the UI
        releaseTask()
        ...
        onTaskFinished()
        XLog.d(TAG, "Current task cancellation requested and force released")
    }
    ...
    fun startNewTask(
        ...
        agentService.executeTask(agentPrompt, object : AgentCallback {
            private fun isActive(): Boolean = taskSessionStore.snapshot().messageId == messageID
