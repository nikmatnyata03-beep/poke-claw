...
class DefaultAgentService : AgentService {
    ...
    companion object {
         ...
         */
        private const val LOCAL_TASK_PROMPT = """Anda adalah asisten ponsel. Anda mengontrol ponsel Android menggunakan alat. Pengguna memberi Anda tugas — selesaikan.

## Cara kerja
1. Panggil get_screen_info untuk melihat apa yang ada di layar
2. Putuskan alat mana yang akan digunakan
3. Panggil alat tersebut
4. Periksa hasilnya, lalu putuskan langkah selanjutnya
5. Saat selesai, panggil finish(summary="apa yang anda lakukan atau temukan")

## Panduan pemilihan alat
- Buka aplikasi → open_app(package_name="com.example.app")
- Ketuk (tap) sesuatu → tap_node(node_id="n3") atau tap(x=500, y=300)
- Ketik teks → input_text(text="halo") atau input_text(text="halo", node_id="n5")
- Tekan back/home/enter → system_key(key="back")
- Scroll untuk mencari sesuatu → scroll_to_find(text="Pengaturan")
- Cari dan ketuk teks → find_and_tap(text="Kirim")
- Kirim pesan → send_message(contact="Ibu", message="hai", app="WhatsApp")
- Panggilan telepon → make_call(contact="Ibu")
- Cek baterai/wifi/storage/bluetooth/screen/device/time → get_device_info(category="battery")
- Baca notifikasi → get_notifications()
- Baca clipboard → clipboard(action="get")
- Tulis teks ke clipboard → clipboard(action="set", text="...")
- Daftar aplikasi terinstal → get_installed_apps()
- Ambil screenshot → take_screenshot()
- Tunggu loading → wait(duration_ms=2000)

## Aturan
- Satu panggilan alat per putaran. Periksa layar setelah setiap tindakan.
- Jika ada yang tidak berhasil, coba pendekatan lain. Setelah 3 kegagalan, panggil finish dan jelaskan apa yang salah.
- finish(summary) harus berisi DATA SEBENARNYA yang diminta pengguna. "Baterai di 73%" bukan "Saya mengecek baterai."
- Gunakan get_device_info untuk kueri battery/wifi/storage/bluetooth/screen/device/time. JANGAN buka aplikasi Pengaturan untuk hal ini.
- Gunakan get_notifications untuk membaca notifikasi. JANGAN tarik panel notifikasi ke bawah.
- Gunakan clipboard(action="get") HANYA saat pengguna bertanya tentang isi clipboard SAAT INI (misalnya "baca clipboard saya" atau "apa yang saya salin").
- Jika pengguna meminta Anda menyalin/mencari/mengirim/meringkas informasi DARI sumber lain (email, browser, catatan, pesan, layar, dll), pertama-tama buka sumber tersebut dan cari informasinya di sana. JANGAN asumsikan itu sudah ada di clipboard.
- Jika Anda membutuhkan clipboard setelah menemukan data sumber, gunakan clipboard(action="set", text="...") sendiri.
- Gunakan get_installed_apps() saat pengguna bertanya aplikasi apa saja yang terinstal.
- Gunakan input_text untuk mengetik. JANGAN ketuk pada saran pelengkapan otomatis.
- Jangan pernah mengatakan Anda tidak dapat mengakses clipboard, notifikasi, atau status ponsel pengguna saat alat yang cocok tersedia. Gunakan alatnya dulu.
- JANGAN isi otomatis kata sandi, konfirmasi pembayaran, atau hapus data."""
    ...
    private fun buildDeviceContext(): String {
        ...
        sb.append("\n\n## Info Perangkat\n")
        sb.append("- Merek: ").append(Build.BRAND).append("\n")
        sb.append("- Model: ").append(Build.MODEL).append("\n")
        sb.append("- Versi Android: ").append(Build.VERSION.RELEASE)
        ...
        try {
            ...
            sb.append("- Resolusi Layar: ").append(dm.widthPixels).append("x").append(dm.heightPixels).append("\n")
        } catch (e: Exception) {
            XLog.w(TAG, "Failed to get display metrics", e)
        }

        sb.append("- Alat Terdaftar: ").append(ToolRegistry.getAllTools().size).append("\n")
        ...
        sb.append("\n## Info Aplikasi Ini\n")
        sb.append("- Nama Aplikasi: ").append(appName).append("\n")
        sb.append("- Nama Paket: ").append(app.packageName).append("\n")
        sb.append("- Ketika pengguna merujuk ke 'aplikasi ini', yang mereka maksud adalah aplikasi di atas.\n")
     ...
     * - Protected zone (most recent config.keepRecentRounds rounds): keep intact
    ...
    private fun compressHistoryForSend(messages: MutableList<ChatMessage>) {
        ...
        if (aiIndices.size <= config.keepRecentRounds) return

        val totalRounds = aiIndices.size

        for (roundIdx in aiIndices.indices) {
            val roundFromEnd = totalRounds - roundIdx
            if (roundFromEnd <= config.keepRecentRounds) break // protected zone
    ...
    private fun runAgentLoop(userPrompt: String, callback: AgentCallback) {
        ...
        val promptForModel = if (parsedPrompt.hasChatHistory || parsedPrompt.hasBackgroundState) {
            buildString {
                append("Anda melanjutkan ruang obrolan (chatroom) yang ada. Gunakan konteks yang disediakan saat permintaan saat ini merujuk ke pesan sebelumnya atau menanyakan aktivitas latar belakang saat ini.\n\n")
                parsedPrompt.backgroundState?.trim()?.takeIf { it.isNotEmpty() }?.let { state ->
                    append("Status latar belakang saat ini:\n")
                    append(state)
                    append("\n\n")
                }
                parsedPrompt.chatHistory?.trim()?.takeIf { it.isNotEmpty() }?.let { history ->
                    append("Ruang obrolan sejauh ini:\n")
                    append(history)
                    append("\n\n")
                }
                append("Permintaan pengguna saat ini:\n")
