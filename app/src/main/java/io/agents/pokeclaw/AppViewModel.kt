...
class AppViewModel : ViewModel() {
    ...
    private val _isFlashTaskMode = kotlinx.coroutines.flow.MutableStateFlow(io.agents.pokeclaw.utils.KVUtils.isFlashTaskMode())
    val isFlashTaskMode: kotlinx.coroutines.flow.StateFlow<Boolean> = _isFlashTaskMode.asStateFlow()

    fun setFlashTaskMode(flash: Boolean) {
    ...
    fun getAgentConfig(): AgentConfig =
        ModelConfigRepository.snapshot().toAgentConfig(
            temperature = 0.1,
            maxIterations = if (isFlashTaskMode.value) 10 else 1000,
            keepRecentRounds = if (isFlashTaskMode.value) 1 else 3
        )
