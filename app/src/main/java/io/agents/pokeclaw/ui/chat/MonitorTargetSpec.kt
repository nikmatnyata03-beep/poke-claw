...
data class MonitorTargetSpec(
    ...
    companion object {
        val supportedApps = listOf("WhatsApp", "WhatsApp Business", "Telegram", "Messages", "LINE", "WeChat")
    }
}
...
object MonitorTargetParser {
    ...
    private fun extractApp(text: String): String {
        ...
        return when {
            Regex("""\bwhatsapp\s+business\b|\bwa\s+business\b""").containsMatchIn(lower) -> "WhatsApp Business"
    ...
    private fun extractLabel(text: String): String {
        ...
        val removeWords = listOf(
            "monitoring", "monitor", "auto-reply", "auto reply", "watching", "watch", "pantau",
            "on whatsapp business", "on wa business", "on whatsapp", "on telegram", "on messages", "on google messages", "on sms", "on wechat", "on line",
