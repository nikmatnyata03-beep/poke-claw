...
object FloatingCircleManager {
    ...
    fun updateTokenStatus(step: Int, formattedTokens: String, formattedCost: String, tokenState: io.agents.pokeclaw.agent.TokenMonitor.State) {
        mainHandler.post {
            ...
            val view = EasyFloat.getFloatView(FLOAT_TAG) ?: return@post
            val tvStatus = view.findViewById<TextView>(R.id.tvTokenStatus) ?: return@post
            tvStatus.text = "Step $step | $formattedTokens | $formattedCost"

            // Update pill color based on token state
            val cardRunning = view.findViewById<androidx.cardview.widget.CardView>(R.id.cardRunning) ?: return@post
    ...
    private fun getNavigationBarHeight(): Int {
        val app = appRef ?: return 0
        val resourceId = app.resources.getIdentifier("navigation_bar_height", "dimen", "android")
